...
input_num = 312051
# Find the next 'pivot'/'corner'. We know that all the lower right endpoints are odd squares, so we find the closest square to our input, rounding upwards.
...
# Our number has the advantage over a square in that we have, in a sense, travelled some of the distance already. Therefore, this could also be written as just the distance_from_ring_pivot, but hindsight is 20/20.
...
while max_value <= input_num:
	...
	max_value += matrix[(current_x + 1), (current_y + 1)] + matrix[(current_x + 1, current_y - 1)]
	matrix[(current_x, current_y)] = max_value
